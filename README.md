...
### 短期目标（v0.2.0）
- 完善工具调用功能（2026/2/14）
    - write_file, 删除文件的同时要删掉标签栏的显示(标签页状态管理 && 四个文件操作工具)
        - 主要问题应该是标签页状态管理 对 工具操作标签页的支持不足吧（√）
        - 搜索工具，使用novel不能搜到根目录，应该留空才行（√）
        - 其他类似问题？比如标签栏多了怎么处理，标签多了运转还正常吗？（）
        - write_file如果路径过长，会发生什么？（）
    - 四个文件操作工具，需要支持用户介入（）
