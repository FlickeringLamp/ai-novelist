...
- 完善工具调用功能（2026/2/13）
    - 修复其他工具
        - 阅读所有工具文件的代码，测试其他工具是否正常使用
        - 统一所有的“行”为“段”（√）
        - 所有的工具，使用统一的段落切分函数（√）
        - read_file，看起来没有问题（√）
        - write_file, 允许填null来删除文件（√）
            - 删除的同时要删掉标签栏的显示（这个牵扯到标签管理，比较复杂，先不处理）
        - 所有文件操作工具使用file_service（√）
        - 所有工具改成异步（√）
        - read_file有问题,指定段落读取时，会从1开始分配段落数（√）
        - search_file测试（）
        - search_and_replace测试（）
        - 探索@tool()的其他参数————其实直接问问ai就知道怎么个回事了，到底有没有重叠很清楚。
    - 四个文件操作工具，需要支持用户介入（）
- 完成聊天栏全部功能
    - 输入栏的自动补全文件路径
    - 选择模式按钮
...
- 工具加强
    ...
    - write_file工具，当内容填null时，看作“删除该文件”
