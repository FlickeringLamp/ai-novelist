...
- 完善工具调用功能（2026/2/14）
    - write_file工具
        - 标签栏多了怎么处理，先看看两个栏的情况会创到哪里吧（）————先把历史记录管理做好吧，每次删东西重测太浪费时间了。
    - **历史消息板块**
        - 首先支持新建会话的前后端功能，即读取配置的会话id，新建会话id（√）
        - 然后修复对话历史api（）
        - 前端仿照roo code，在直接聊天前，添加一个流程：选择历史会话/直接聊天开新会话()
        - 历史消息面板（折叠状态）当用户发送消息时/选定一个id后加载state，是否会消失？
            - 修后端api的端口名，从new_thread改update_thread
            - 先删掉挂载自动加载配置文件里的thread_id机制，直到用户直接发送消息才自动创建新的thread_id并更新旧的
            - 然后创建消息，获取state（直发）/加载state，无论是哪一种情况，只要有消息，就要隐藏对话历史栏
    - 其他文件操作工具
    - 四个文件操作工具，需要支持用户介入（）
    - 没有任何知识库，添加知识库时，没有提供商apikey内容会报错，删不掉写入的错误配置，也创建不了新的配置
    - 临时添加工具结果/及时获取工具结果并渲染/中断返回结果时先返回state
...
- 完成聊天栏全部功能
    ...
    - 中断ai的流式消息
    - 输入栏，在“确认/取消”时，直接回车发送的情况需要进一步打磨。
- 部分功能自动化测试
