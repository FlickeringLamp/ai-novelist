...
*   **回档**: 消息自动存档，随时修改目标气泡。文件手动存档，允许进度回退。
...
- 重构前端章节文件板块
    ...
    - 文件回档&前端的差异对比（专门做一个差异对比页面，和普通标签页分离，脏数据的状态显示有所区别，允许仅展示用的页面锁定，可能需要处理前端的状态管理）
    - 不再自动创建默认文件名，默认为空供前端输入，包括后缀（）
    - 命令行输入栏
    - skills,脚本的python npm等，应该优先使用项目自带的，而不是去找用户电脑里的
    - 路径补全
    - 拖动移动功能，修新建文件按钮
    - 前端，正在生成时，标注气泡（比如绿色边框）
    - 前端表单/手动填写，写好skills的名字，决定是否启用skills，以及填写env
