...
- **聊天栏功能不完善**：目前回档，文件路径补全，中断AI流式输出等，暂未实现
...
- 完善工具调用功能（2026/2/21）
    ...
    - 回档对话（）
    - 中断ai的流式消息（）
    - 修复AI有时调用工具进入无限循环，无法中断的问题。
        - 修复思路：1. 强制停止 2. 修改graph，在解除中断的原语里，同时触发用户消息
        - 先实现2思路吧
...
- 工具加强
    ...
    - 多个ai同时修改文件（？）
    - bug————当ai先前使用过工具，后续关闭，继续对话时————一旦ai调用这个关闭的工具，就会报错。但是可控，渲染崩溃后，打开消息，删掉最后一个调用就好。
    - 临时添加工具结果/及时获取工具结果并渲染/中断返回结果时先返回state，让工具结果消息先渲染，再流式渲染中断消息（）
- 总结对话历史()————存在点小问题，比如，保留了倒数第二条消息，但是却显示没有发给ai
- qwen3max，工具调用有问题，可能是模型原因，也可能是ChatTongyi的问题，或许换成ChatQwen能解决？（以后再试试）
