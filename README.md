...
- 完善工具调用功能（2026/2/13）
    - 修复其他工具
        - write_file, 允许填null来删除文件（√）
            - 删除的同时要删掉标签栏的显示（这个牵扯到标签状态管理，比较复杂，先不处理）
        - search_file测试（√）
            - 应该返回相对novel的路径，而不是这种：backend\data\novel\test.md:23（暂不影响核心功能，跳过）
        - ask_user测试（√），新增前端的ask工具显示
        - 两个emb工具（√）
            - 修复功能，将它们从注释状态恢复，适应新的后端(√)
            - 把知识库列表做进系统提示词呢？（√）
            - 两步rag初步实现（√）
        - 探索@tool()的其他参数，尤其是description————其实直接让ai输出自己的提示词就知道怎么个事了，到底有没有提示词重叠很清楚。
    - 四个文件操作工具，需要支持用户介入（）
- 完成聊天栏全部功能
    - 输入栏的自动补全文件路径
    - 选择模式按钮
