...
- 重构前端章节文件板块
    ...
    - 路径补全 ———— 引用的文件内容被放在提示词里面了，没有持久化逻辑，第二轮对话，ai就忘记东西了。
    - 拖动移动功能 ———— 需要完整重新设计元素渲染，实现嵌套结构，才能解决拖动问题
    - 修新建文件按钮（后续再说）———— 再问删了()
    - 前端表单/手动填写，写好skills的名字，决定是否启用skills，以及填写env ———— 暂时跳过,或者使用内置agent来做
    - 限制ai工具，只能每次获取100个文件等。———— 暂时跳过
    - 允许手动介入差异对比视图 ———— 必做
    - 重构，统一类型声明位置
