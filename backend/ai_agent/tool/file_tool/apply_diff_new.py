...
class LineReplacement(BaseModel):
    ...
    new: str = Field(description="替换后的新内容（单行文本）")
...
def apply_diff_new(path: str, replacements: List[LineReplacement]) -> str:
    """应用差异修改
    ...
    # 构造包含工具具体信息的中断数据
    interrupt_data = {
        "tool_name": "apply_diff_new",
        "tool_display_name": "应用差异（简化版）",
        "description": f"应用差异: {path}",
        "parameters": {
            "path": path,
            "replacements": [{"line": r.line, "old": r.old[:50] + "..." if len(r.old) > 50 else r.old, "new": r.new[:50] + "..." if len(r.new) > 50 else r.new} for r in replacements]
        }
    }
    ...
    if choice_action == "1":
        try:
            ...
            # 按行号排序（从小到大，因为每行只替换一行，不会偏移）
            sorted_replacements = sorted(replacements, key=lambda x: x.line)
