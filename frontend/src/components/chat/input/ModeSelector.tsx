...
const ModeSelector = () => {
  ...
  useEffect(() => {
    const loadModeSettings = async () => {
      try {
        // 加载当前模式
        const currentModeValue = await httpClient.get(`/api/config/store?key=${encodeURIComponent('currentMode')}`);
        if (currentModeValue) {
          setCurrentMode(currentModeValue);
        }
        
        // 加载自定义模式
        const customModesValue = await httpClient.get(`/api/config/store?key=${encodeURIComponent('customModes')}`);
        if (customModesValue && Array.isArray(customModesValue)) {
          setCustomModes(customModesValue);
        }
      } catch (error) {
  ...
  const allModes: Mode[] = [
    ...builtInModes,
    ...(Array.isArray(customModes) ? customModes.map((mode: Mode) => ({
      id: mode.id,
      name: mode.name,
  ...
  // 过滤模式
  const filteredModes = allModes.filter((mode: Mode) =>
  ...
  useEffect(() => {
    const handleClickOutside = (e: MouseEvent) => {
      if (isExpanded && !(e.target as HTMLElement).closest('.mode-selector-container')) {
        setIsExpanded(false);
        setSearchText('');
        setPage(0);
      }
