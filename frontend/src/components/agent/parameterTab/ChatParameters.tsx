...
interface ModeParameters {
  [key: string]: AIParameters;
}

interface AdvancedSettingsProps {
  aiParameters?: ModeParameters;
  onParametersChange?: (mode: string, parameters: AIParameters) => void;
  mode?: string;
}
...
const AdvancedSettings = ({
  ...
  // 本地状态管理
  const [localParameters, setLocalParameters] = useState<AIParameters>({});
  ...
  // 处理AI参数变化
  const handleParameterChange = (parameter: keyof AIParameters, value: number) => {
    const newParameters = {
      ...localParameters,
      [parameter]: value
    };
  ...
  useEffect(() => {
    const updateSliderProgress = () => {
      ...
      sliders.forEach(slider => {
        ...
        inputElement.style.setProperty('--slider-progress', `${progress}%`);
      });
    };
    ...
    <div className="p-5 bg-theme-black text-theme-white">
      ...
      >
        ...
        <SliderComponent
          ...
          max={1}
          step={0.1}
          onChange={(value: number) => handleParameterChange('top_p', value)}
        ...
        <SliderComponent
          ...
          max={5}
          step={1}
          onChange={(value: number) => handleParameterChange('n', value)}
        ...
        <InputComponent
          ...
          onChange={(value: number) => handleParameterChange('max_tokens', value)}
