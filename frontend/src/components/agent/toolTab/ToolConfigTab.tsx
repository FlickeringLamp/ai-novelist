...
interface ToolConfigTabProps {
  mode: string;
  modeType: string;
  onToolConfigChange?: (mode: string, newConfig: ToolConfig) => void;
}
 ...
 * 负责管理每个模式的工具启用状态
 */
const ToolConfigTab = ({ mode, modeType, onToolConfigChange }: ToolConfigTabProps) => {
  // 状态管理
  ...
  const [originalConfig, setOriginalConfig] = useState<ToolConfig | null>(null);
  ...
  // 通知父组件配置变更
  const notifyConfigChange = (newConfig: ToolConfig) => {
  ...
  // 切换整个工具分类
  const toggleToolCategory = (categoryName: string) => {
    const categoryTools = toolConfig.tool_categories[categoryName as keyof typeof toolConfig.tool_categories] || [];
    ...
    if (allEnabled) {
      // 如果分类中所有工具都已启用，则禁用所有
      categoryTools.forEach((tool: string) => {
  ...
  // 检查分类是否全部启用
  const isCategoryAllEnabled = (categoryName: string) => {
    const categoryTools = toolConfig.tool_categories[categoryName as keyof typeof toolConfig.tool_categories] || [];
    return categoryTools.length > 0 &&
           categoryTools.every((tool: string) => toolConfig.enabled_tools.includes(tool));
  };

  // 检查分类是否部分启用
  const isCategoryPartialEnabled = (categoryName: string) => {
    const categoryTools = toolConfig.tool_categories[categoryName as keyof typeof toolConfig.tool_categories] || [];
    const enabledCount = categoryTools.filter((tool: string) =>
      toolConfig.enabled_tools.includes(tool)
    ).length;
  ...
  return (
    <div className="p-5 max-h-[600px] overflow-y-auto">
      ...
      <div className="flex flex-col gap-5">
        {Object.entries(toolConfig.tool_categories).map(([categoryName, tools]: [string, string[]]) => (
